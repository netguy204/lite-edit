---
decision: FEEDBACK
summary: "Iteration 1 feedback not addressed: stop() still uses SIGKILL directly instead of SIGTERM→wait→SIGKILL"
...
- **Status**: gap
- **Evidence**: `AgentHandle::stop()` (lines 449-464) calls `terminal.kill()` which sends SIGKILL (see `pty.rs` line 172 comment: "This sends SIGKILL"). The GOAL.md criterion states "sends SIGTERM" and PLAN.md specifies: "Send SIGTERM, wait briefly, SIGKILL if needed." The implementation was flagged in iteration 1 but remains unchanged.
...
### Issue 1: stop() still uses SIGKILL instead of SIGTERM (UNADDRESSED FROM ITERATION 1)

- **id**: issue-sigterm-unaddressed
- **location**: `crates/terminal/src/agent.rs:449-464`
- **concern**: This is the SAME issue from iteration 1. The `stop()` method sends SIGKILL directly via `terminal.kill()` instead of following the SIGTERM→wait→SIGKILL pattern specified in both GOAL.md ("Stopping a running agent sends SIGTERM") and PLAN.md step 4. This prevents agents like Claude Code from performing graceful cleanup (saving state, closing file handles, etc.).
- **suggestion**: Implement the specified behavior:
  1. Add `nix` crate dependency for POSIX signal support
  2. Get process ID from the PTY child handle
  3. Send SIGTERM via `nix::sys::signal::kill(pid, Signal::SIGTERM)`
  4. Wait briefly (100ms) polling `try_wait()` for exit
  5. If still running after timeout, send SIGKILL via `terminal.kill()`
  6. Transition to `Exited { code: -15 }` if SIGTERM worked, or `-9` if SIGKILL was needed
- **severity**: functional
- **confidence**: high

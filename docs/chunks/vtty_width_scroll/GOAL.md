...
Fix two related VTTY rendering bugs that degrade the terminal experience:

1. **PTY width mismatch**: The PTY's column count appears to be set wider than the actual visible character columns in the terminal pane. Programs like `git status` format output for the wider PTY width, but the display area is narrower, causing excessive soft-wrapping with odd blank lines between entries. The user's hypothesis is that the VTTY is being given a window width that is too wide.

2. **Scroll can't reach bottom**: After running multiple commands in the terminal, the user cannot scroll to the bottom to see the latest prompt. The viewport's maximum scroll offset appears to be too low, leaving the user stuck viewing partial output from earlier commands.

These bugs directly undermine the "embed a fully-functional terminal" goal — users cannot trust the terminal for basic shell workflows if output wraps incorrectly and they lose the ability to see their prompt.

## Observed Behavior

**Screenshot 1 (wrapping)**: Running `git status` produces output with excessive blank lines between file entries. The filenames are short enough to fit on a single line, yet the output appears to wrap as if the terminal were much narrower than it appears — consistent with git formatting for a wider PTY and the display then hard-wrapping at the actual visible width.

**Screenshot 2 (scroll)**: After running `git status` three times and scrolling as far down as possible, the prompt is not visible. The viewport stops partway through the third `git status` output, suggesting the scroll max is calculated with a stale or incorrect line count.

## Investigation Notes

### PTY Width Flow

In `editor_state.rs:919-920`, the PTY column count is calculated as:
```rust
let cols = (pane_width as f64 / advance_width).floor() as usize;
```

Where `pane_width` comes from `pane_rect.width` (pixel width of the pane content area). Possible root causes:
- `advance_width` (the font's character advance) could be wrong or stale
- `pane_width` could include padding, margins, or separator space that the renderer doesn't actually use for characters
- There could be a mismatch between how the renderer lays out characters and how `cols` is calculated

### Scroll Clamping Flow

The scroll max is computed in `row_scroller.rs:117-121`:
```rust
let max_rows = row_count.saturating_sub(self.visible_rows);
let max_offset_px = max_rows as f32 * self.row_height;
```

The `row_count` for terminals comes from `terminal.line_count()` (history + screen lines). The viewport is updated with the current line count in `sync_pane_viewports()` (called on resize) and in `poll_standalone_terminals()` (called on PTY output with auto-follow).

However, auto-follow only runs `scroll_to_bottom()` when the viewport was already at the bottom. If the user has manually scrolled up, subsequent manual scrolling back down is clamped by `set_scroll_offset_px()` using whatever `row_count` was passed — which may be stale if the viewport's cached size hasn't been refreshed since the terminal's line count grew.

Key question: when the user scrolls manually (mouse wheel / trackpad), what `row_count` is passed to `set_scroll_offset_px()`? If it's a cached value from the last `sync_pane_viewports()` call (which only runs on resize events), the clamp will be too restrictive.

## Success Criteria

- Running `git status` (or any shell command) in a terminal pane produces output that wraps at the correct column — matching the visible character width of the pane
- After running multiple commands and scrolling up, the user can scroll back down to see the prompt at the bottom of the terminal output
- Add a test verifying PTY cols match the actual character capacity of the pane's rendering area
- Add a test verifying scroll max is computed with the terminal's current (not stale) line count
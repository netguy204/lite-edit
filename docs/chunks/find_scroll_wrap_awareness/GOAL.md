...
Fix find-in-file (Cmd+F) scrolling so the highlighted match is actually visible
in the viewport when soft wrapping is active.

Currently, `run_live_search()` in `editor_state.rs` (line ~2019) calls
`ensure_visible_with_margin(match_line, line_count, 1)` which delegates to
`RowScroller::ensure_visible_with_margin()` — a logical-line-based scroller
that assumes 1 buffer line = 1 screen row. When soft wrapping is enabled, lines
before the match may occupy multiple screen rows, so the calculated scroll
position is too low, placing the match 2-3 lines below the visible area.

The fix is to use the **wrap-aware** scrolling path that already exists for
cursor following: `ensure_visible_wrapped()` in `viewport.rs` (line ~299).
This method computes absolute screen rows by summing wrapped line heights and
accounts for column position within wrapped lines.

### Root cause

`run_live_search()` and `advance_to_next_match()` pass only the buffer line
number to the unwrapped `ensure_visible_with_margin()`. They need to instead
use `ensure_visible_wrapped()` (or a new margin-aware variant of it) that:

1. Sums `wrap_layout.screen_rows_for_line()` for all lines before the match
2. Adds `wrap_layout.buffer_col_to_screen_pos(match_col).0` for the match's
   sub-row within its wrapped line
3. Applies the margin (currently 1 row for find strip clearance)

## Success Criteria

- When soft wrapping is active and a find match is on or after wrapped lines,
  the viewport scrolls so the match highlight is **visible** within the viewport
- The match is not obscured by the find strip (margin still respected)
- Scrolling to matches still works correctly when wrapping is disabled
- Both `run_live_search()` (live incremental search) and
  `advance_to_next_match()` (Return key) use wrap-aware scrolling

...
When a new workspace is created — whether at application startup or via the
user-triggered new-workspace action — its initial tab should be a terminal tab
spawned in the workspace's `root_path`, rather than an empty file tab showing
the welcome screen.

This makes the editor feel immediately useful: the user lands in a shell in
their project directory, ready to run commands, without needing to manually
open a terminal tab.

The two code paths that create a workspace with an initial tab are:

1. **`EditorState::new_workspace()`** (`editor_state.rs:2625`) — user-triggered
   via the directory picker. At this point view dimensions are valid, so
   `new_terminal_tab()` can be called directly after workspace creation.

2. **`EditorState::add_startup_workspace()`** (`editor_state.rs:388`) — called
   at application startup. Critically, `update_viewport_dimensions()` is called
   *after* this function, so view dimensions are still zero at call time. A
   deferred or default-dimension approach is required here.

The challenge is handling the startup case where terminal dimensions aren't
yet known. The implementation must ensure that the terminal is spawned in all
cases, even when view dimensions are zero at workspace-creation time.

## Success Criteria

- When the user triggers "New Workspace" (via directory picker), the new
  workspace opens with a single terminal tab running in the selected directory.
  No empty file tab / welcome screen is shown.
- At application startup, after the directory picker resolves and the workspace
  is created, the initial tab is a terminal tab in that directory. No empty file
  tab / welcome screen is shown.
- The terminal label follows the existing naming convention: "Terminal" for the
  first terminal tab in the workspace.
- Existing tests that rely on a new workspace starting with an empty file tab
  are updated to reflect the new behavior.
- The welcome screen (`should_show_welcome_screen()`) no longer appears on
  workspace creation (it will still be available for explicitly empty file tabs).

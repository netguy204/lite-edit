...
Make the first tab displayed on program startup show the welcome screen instead of demo text.

The welcome_screen chunk successfully implemented logic to show a polished Vim-style welcome screen when a file tab has an empty buffer. However, when the editor launches, it still initializes the first tab with demo content (`generate_demo_content()`), so the welcome screen never appears on startup.

This chunk fixes the disconnect by:
1. Initializing the first buffer as empty instead of populated with demo text
2. Leveraging the existing welcome_screen logic that automatically displays the welcome screen for empty file tabs
3. Removing the now-unused `generate_demo_content()` function

This is a consistency/UX polish fix: users launching lite-edit for the first time should see the professional welcome screen in their initial tab, not demo code. The new tab experience (Cmd+T) already shows the welcome screen because those tabs are created empty; this makes the startup experience consistent.

## Success Criteria

- When the editor launches with no file arguments, the initial tab displays the welcome screen (feather logo, hotkeys, etc.)
- The initial buffer is empty (no demo text content)
- Typing any character in the initial tab dismisses the welcome screen and transitions to normal buffer editing
- Creating a new tab (Cmd+T) still shows the welcome screen (existing behavior is preserved)
- The `generate_demo_content()` function is removed from main.rs (no longer needed)
- No new tests are required—this is leveraging existing welcome_screen functionality that's already tested

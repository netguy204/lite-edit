...
The focus system is a flat enum (`EditorFocus`) with 4 variants: Buffer, Selector, FindInFile, ConfirmDialog. Events go directly to the current focus target with no propagation. This creates several problems:
- Adding a new focusable widget requires modifying the closed `EditorFocus` enum
- Global keyboard shortcuts (Cmd+Q, Cmd+W, Cmd+S) must be duplicated in every focus target implementation
- Nested focus (find bar inside a pane vs. global find) requires special-casing
- No foundation for future accessibility (VoiceOver expects a responder hierarchy)

Replace the `EditorFocus` enum with a `FocusStack` — an ordered stack of `Box<dyn FocusTarget>`. Events propagate top-down: the topmost target gets first crack; if it returns `Handled::No`, the event falls through to the next target. Global shortcuts live at the bottom of the stack and always get a chance to handle events. Overlays (find bar, selector, dialog) push onto the stack and pop when dismissed.

**Key files**: `crates/editor/src/focus.rs` (EditorFocus enum → FocusStack), `crates/editor/src/drain_loop.rs` (event dispatch), `crates/editor/src/editor_state.rs` (holds focus state), `crates/editor/src/buffer_target.rs`, `crates/editor/src/selector.rs`, `crates/editor/src/mini_buffer.rs`, `crates/editor/src/confirm_dialog.rs` (all FocusTarget impls)

**Origin**: Architecture review recommendation #5 (P1 — Architecture). See `ARCHITECTURE_REVIEW.md`.

## Success Criteria

- `EditorFocus` enum replaced with `FocusStack` (ordered `Vec<Box<dyn FocusTarget>>`)
- `FocusTarget::handle_key` returns `Handled` enum (`Yes` / `No`) to enable propagation
- Global shortcuts (Cmd+Q, Cmd+W, Cmd+S, Cmd+N, etc.) handled by a single `GlobalShortcutTarget` at the stack bottom — not duplicated per target
- Overlays push onto stack on open, pop on dismiss (find bar, selector, confirm dialog)
- Buffer focus target is always present as the base editing layer
- Event propagation is top-down (most recently pushed target handles first)
- Existing keyboard behavior unchanged for all current interactions
- New focus targets can be added without modifying any enum

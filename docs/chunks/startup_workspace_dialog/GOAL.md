...
3. If the user cancels the dialog, either quit the application or create a workspace with no file index active (deferred until a directory is chosen).
...
- If the user cancels the picker, the application exits gracefully (or opens with an empty workspace that does not index any directory).

...
Every tab holds its buffer as `Arc<Mutex<Box<dyn BufferView>>>`. During rendering, the mutex is acquired for each `styled_line()` call — once per visible line. For a 40-line viewport, that's 40 lock/unlock cycles per frame (~1µs average, up to 40µs P99 under contention with the PTY reader thread).

Restructure the render path to lock each tab's buffer exactly once per frame, extract all needed data (styled lines, cursor info, dirty state) under a single lock, then release before quad emission. This reduces 40 mutex operations to 1 per tab.

Additionally, consider whether the `Box<dyn>` inside `Arc<Mutex<>>` can be eliminated — `Arc<Mutex<dyn BufferView>>` achieves the same polymorphism with one fewer heap allocation and one fewer pointer indirection per access.

**Key files**: `crates/editor/src/renderer.rs` (render loop that accesses tab buffers), `crates/editor/src/workspace.rs` (Tab struct with Arc<Mutex<Box<dyn BufferView>>>)

**Origin**: Architecture review recommendation #4 (P1 — Performance). See `ARCHITECTURE_REVIEW.md`.

## Success Criteria

- Each tab's buffer mutex is locked at most once per render frame
- All data needed for rendering (styled lines, cursor, dirty state) extracted under single lock
- Quad emission and Metal encoding happen after lock release
- No deadlocks or data races introduced (PTY reader can still push data between frames)
- Measurable: mutex lock count per frame reduced from ~40 to 1 per visible tab
- Terminal rendering still works correctly (PTY data arriving during render waits for next frame)
- Optional: `Box<dyn BufferView>` inside `Arc<Mutex<>>` eliminated in favor of `Arc<Mutex<dyn BufferView>>`

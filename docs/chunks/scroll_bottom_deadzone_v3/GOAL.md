...
The `scroll_bottom_deadzone` and `scroll_wrap_deadzone_v2` chunks attempted to fix scroll deadzone and click-to-cursor misalignment at the bottom of files. After implementing both, all originally reported symptoms persist:

### Observed Symptoms (still present)

**"Deadzone" defined**: A phantom scroll region at the bottom of the document where the logical scroll offset continues to advance but the view does not visually move. The user scrolls and nothing happens on screen. They must then "unwind" that phantom distance by scrolling back up before the view begins to respond again.

1. **1-line phantom scroll region with no wrapping**: When scrolling to the bottom of a document with no wrapped lines, the user can visually reach the last line of the document. However, approximately one line height of additional scroll input is accepted without any visual effect. Scrolling back up requires unwinding this phantom distance before the view moves.

2. **Larger phantom scroll region + unreachable content with wrapping**: When the window width is adjusted so that lines soft-wrap, two things happen: (a) the phantom scroll region grows proportionally to the number of wrapped lines, and (b) scrolling visually stops *before the final lines of the document are visible*. The user cannot scroll to see the true bottom of the file.

3. **Click-to-cursor off-by-one in phantom region (no wrapping)**: When in the 1-line phantom region (no wrapped lines), clicking positions the cursor 1 line below where the user clicked.

4. **Click-to-cursor severe misalignment in phantom region (wrapping)**: When in the multi-line phantom region (caused by soft-wrapped lines), the cursor does not immediately appear when clicking. If the user scrolls after clicking, the cursor appears but is positioned multiple lines below the click point.

### Likely Root Causes

The previous two chunks identified the right general problem area (scroll clamping must account for screen rows vs buffer lines) but did not fully resolve the coordinate mismatches. Key areas to investigate:

- **`pixel_to_buffer_position_wrapped`**: May use `first_visible_line()` (a screen row index) as if it were a buffer line index, causing click-to-cursor to walk from the wrong starting line
- **`set_scroll_offset_px_wrapped` / `compute_total_screen_rows`**: The max offset calculation may still disagree with the renderer's actual content height, allowing scroll past the true bottom
- **The baseline 1-line phantom region without wrapping**: This suggests a fundamental off-by-one in `set_scroll_offset_px` (the non-wrapped path), independent of any wrap-related issues. The non-wrapped clamping formula `max = (row_count - visible_rows) * row_height` may itself be off by one.
- **Unreachable content with wrapping**: The max scroll offset is computed using total screen rows, but this value is larger than the renderer's actual content height. This means the scroll clamping allows offsets that go past the renderable area, while simultaneously the renderer stops drawing before the true bottom because it has run out of viewport space. The excess max offset becomes the phantom region, and the content that should have been visible at the true max is unreachable.

## Success Criteria

- Scrolling to the bottom of a file with **no wrapped lines**: the last line is visible, no further scroll input is accepted (no phantom region), and scrolling back up responds immediately.
- Scrolling to the bottom of a file with **wrapped lines**: the last line is visible (content is not cut off), no further scroll input is accepted, and scrolling back up responds immediately.
- Clicking in the buffer at any scroll position — including maximum — places the cursor at the line the user clicked, not offset by any number of lines.
- All existing scroll and viewport tests continue to pass.

## Relationship to Parent

This chunk supersedes the work of both `scroll_bottom_deadzone` and `scroll_wrap_deadzone_v2`. Those chunks correctly identified the problem domain (scroll clamping and coordinate mapping at the bottom of files) but did not resolve the symptoms.

What remains valid from the parent chunks:
- The diagnosis that scroll clamping must account for wrapped lines
- The general approach of using total screen rows for wrap-aware clamping
- Helper functions like `buffer_line_for_screen_row` and `compute_total_screen_rows`
- Test structure and test cases (though assertions may need correction)

What needs to change:
- The actual clamping math — both the wrapped and non-wrapped paths still produce incorrect max offsets
- The coordinate mapping in click handling — `pixel_to_buffer_position_wrapped` likely has a screen-row-vs-buffer-line confusion
- Possibly `visible_lines()` calculation or its relationship to the max offset formula